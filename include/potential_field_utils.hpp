...
inline cv::Mat
rgb_to_hsv(const cv::Mat &rgb) {
  cv::Mat hsv(rgb.size(), CV_8UC3);
  cv::cvtColor(rgb, hsv, CV_RGB2HSV);
  return hsv;
}

inline cv::Mat
bgr_to_hsv(const cv::Mat &rgb) {
  cv::Mat hsv(rgb.size(), CV_8UC3);
...
/**
 * Calculates the vector field based on hsv pixelvalue
 * @param image bgr image
 * @return cv::Mat vectorfield
 */
cv::Mat
image_to_vectorfield(const cv::Mat &image,
                     float speed_scale=1.0,
                     float min_speed=0.0){
  cv::Mat hsv[3];
  cv::flip(image, image, 1);
  cv::split(bgr_to_hsv(image),hsv);

  cv::Mat angle(hsv[0].rows, hsv[0].cols, CV_32FC1, cv::Scalar(0.0f)),
          speed(hsv[0].rows, hsv[0].cols, CV_32FC1, cv::Scalar(0.0f)),
          vectorFieldX(hsv[0].rows, hsv[0].cols, CV_32FC1, cv::Scalar(0.0f)),
          vectorFieldY(hsv[0].rows, hsv[0].cols, CV_32FC1, cv::Scalar(0.0f)),
          vectorField(hsv[0].rows, hsv[0].cols, CV_32FC2, cv::Scalar(0.0f));
  hsv[0].convertTo(angle, CV_32FC1, 2.0, 0.0);
  hsv[2].convertTo(speed, CV_32FC1, speed_scale, min_speed);
  cv::polarToCart(speed, angle, vectorFieldX, vectorFieldY, true);
...
cv::Point2i pose2pixel(const geometry_msgs::Pose &pose, const int imageWidth, const int imageHeight, const float meterPerPixel) {
  return cv::Point2i(
      (int) (imageWidth/2 + pose.position.x / meterPerPixel),
      (int) (imageHeight/2 + pose.position.y / meterPerPixel));
}
